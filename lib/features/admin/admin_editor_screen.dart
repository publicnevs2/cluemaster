...
import 'package:path/path.dart' as path; // Wichtig für Pfad-Operationen
import 'package:record/record.dart';
import 'package:geolocator/geolocator.dart';
import 'package:permission_handler/permission_handler.dart';
import 'package:uuid/uuid.dart'; // Import für einzigartige Namen
...
class _AdminEditorScreenState extends State<AdminEditorScreen> {
 ...
 final _uuid = Uuid(); // Für einzigartige Dateinamen
 ...
 // NEU: State-Variable für den Pfad des Hintergrundbildes
 ...
 void initState() {
  ...
  if (widget.existingClue != null) {
   ...
   // NEU: Initialisieren des Bildpfades aus dem bestehenden Clue
 ...
 void _save() {
  if (_formKey.currentState!.validate()) {
   ...
   final clue = Clue(
    ...
    // NEU: Den Pfad zum Hintergrundbild hinzufügen
 ...
 // ============================================================
 // NEU: Funktion zum Auswählen des GPS-Hintergrundbilds
 // ============================================================
 Future<void> _pickGpsBackgroundImage() async {
    final XFile? image = await _imagePicker.pickImage(
      source: ImageSource.gallery,
      imageQuality: 80, // Komprimiert das Bild leicht
    );

    if (image == null) return; // User hat abgebrochen

    try {
      final Directory appDir = await getApplicationDocumentsDirectory();
      final String fileExtension = path.extension(image.path);
      // Einzigartiger Dateiname, um Konflikte zu vermeiden
      final String newFileName = '${_uuid.v4()}$fileExtension';
      final String savedImagePath = path.join(appDir.path, newFileName);

      // Kopiere die Datei an den sicheren Ort
      await File(image.path).copy(savedImagePath);

      // Aktualisiere den State, um die UI neu zu zeichnen
      setState(() {
 ...
 Widget _buildGpsRiddleFields() {
  return Column(
   children: [
    ...
    // ============================================================
    // NEU: Hier wird der Abschnitt für das Hintergrundbild eingefügt
    // ============================================================
 ...
 // ============================================================
 // NEU: Das Widget für den GPS-Hintergrund-Abschnitt
 // ============================================================
